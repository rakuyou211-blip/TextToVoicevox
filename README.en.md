...
- **Read from screen** — drag a box around any text on your screen (e-book, PDF viewer, web page) and it's read aloud immediately (Ctrl+R / ⌘R). After turning a page, **↻ 同じ範囲を読む** (Ctrl/⌘+Shift+R) reads the same spot again without re-selecting. Offline, like everything else; macOS needs the one-time *Screen Recording* permission.
